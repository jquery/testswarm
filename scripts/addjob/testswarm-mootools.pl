...
my $JOB_NAME = "MooTools Commit #{REV}";
...
my $SUITE = "http://dev.jquery.com/~john/changeset/mootools/{REV}/Specs/";
sub BUILD_SUITES {
	my $index = `cat $INJECT_FILE`;
	$index =~ s/.*Specs:\s*{(.*?)}.*/$1/s;
	$index =~ s/'\w+'\s*://g;
	%SUITES = map {
		my $file = (glob("Specs/*/$_.js"))[0];
		if ( `grep describe $file` ) {
			$_ => "$SUITE?file=$_";
		}
	} ($index =~ /'([\w.]+)'/g);
}
