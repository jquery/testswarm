...
my $RCS_URL = "git://github.com/sstephenson/prototype.git";

# The directory in which the checkouts will occur.

my $BASE_DIR = "/home/john/public_html/changeset/prototype";

# A script tag loading in the TestSwarm injection script will
# be added at the bottom of the <head> in the following file.

my $INJECT_FILE = "test/unit/tmp/*_test.html";

# Any build commands that need to happen.

my $BUILD = "git submodule init && git submodule update && rake test:build";
...
my $JOB_NAME = "Prototype Commit #{REV}";
...
my $SUITE = "http://dev.jquery.com/~john/changeset/prototype/{REV}";
sub BUILD_SUITES {
	%SUITES = map { /(\w+)_test.html/; $1 => "$SUITE/$_"; } glob($INJECT_FILE);
}
