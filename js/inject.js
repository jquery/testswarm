...
(function (undefined) {
	...
	testFrameworks = {
		...
		'QUnit': {
			...
			install: function () {
				var curModule, curTest, testCount = 0, assertCount = 0, failures = {};

				QUnit.testStart = function(test) {
					testCount++;
					curModule = test.module;
					curTest = test.name;
				};

				QUnit.log = function(test) {
					if (!test.result) {
						if (!failures.hasOwnProperty(curModule)) {
							failures[curModule] = {};
						}

						if (!failures[curModule].hasOwnProperty(curTest)) {
							failures[curModule][curTest] = {};
							failures[curModule][curTest].assertions = []
						}

						test.count = ++assertCount;
						failures[curModule][curTest].count = testCount;
						failures[curModule][curTest].assertions.push(test);
					}
				...
				QUnit.done = function ( results ) {
					submit({
						...
						error: 0,
						total: results.total,
						report_json: JSON.stringify(failures)
					});
				};
