<?php
	...
	if ($pdo->getAttribute(PDO::ATTR_DRIVER_NAME) === 'sqlite') {
		$updated_interval_sql = "DATETIME(run_client.updated, '+5 minutes')";
	} else {
	...
	$sth->execute(array(sql_datetime_now()));
	...
	while ($row = $sth->fetch()) {
		...
		$update_sth->execute(array(sql_datetime_now(), $run_id, $useragent_id));
